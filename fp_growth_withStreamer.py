...
def find_frequent_itemsets(file_path, minimum_support, include_support=False):
    ...
    # items have.
    for transaction in Streamer(file_path):
    ...
    for transaction in imap(clean_transaction, Streamer(file_path)):
...
if __name__ == '__main__':
    ...
    with open('fp_growth_output.csv','w') as outfile:
        for itemset, support in result:
            #print str(itemset) + '|' + str(len(itemset)) + '|' + str(support) + '|' + str(100.0 * int(support) / count_line)
            outfile.write(str(itemset) + '|' + str(len(itemset)) + '|' + str(support) + '|' + str(100.0 * int(support) / count_line) + '\n')
